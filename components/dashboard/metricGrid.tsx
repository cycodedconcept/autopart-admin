...
import { WalletIcon, ShoppingCartIcon, UsersIcon,  WarningTriangleIcon } from '../atoms/Icons';
...
export default function MetricGrid({operational, overview}: {
  operational: OperationalCards | undefined;
  overview: OverviewCards | undefined
}) {

 const {averagePayoutTime, disputeRate, failedPayouts, verificationQueue} = operational ?? {}

const {activeSellers, ordersToday, platformGmv, totalRevenue} = overview ?? {}
...
const grv = {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <MetricCard 
        ...
        value={CurrencyFormat(platformGmv?.currency).format(platformGmv?.trend.currentValue ?? 0)}
         trendImage= {grv}
  ...
  <MetricCard 
      ...
      <MetricCard 
        ...
        value={averagePayoutTime?.valueDays }
      ...
      <MetricCard 
        ...
        value={CurrencyFormat(totalRevenue?.currency).format(totalRevenue?.trend.currentValue ?? 0)}
